...
def _map_binary_series(s: pd.Series) -> pd.Series:
    ...
    Convert categorical features with exactly 2 values into 0/1 integers.
    '''
...
def bulid_features(df: pd.DataFrame, target_col: str = 'Churn') -> pd.DataFrame:
    """
    ...
    # Merge 'No internet/phone service' into 'No' to reduce cardinality.
    # This ensures these features are cleanly treated as binary (Yes=1, No=0)
    # rather than generating redundant dummy variables during One-Hot Encoding.
    df[obj_cols] = df[obj_cols].replace('No internet service','No')
    df[obj_cols] = df[obj_cols].replace('No phone service','No')
